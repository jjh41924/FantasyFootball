...
setPointsForWeek = function(week){
  #Load libraries
  require("XML")
  require("stringr")
  require("ggplot2")
  ...
  #Save file  
  save(projections_espn, file = paste("C:/MY_FF/Weekly Forecast/ESPN/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_espn, file=paste("C:/MY_FF/Weekly Forecast/ESPN/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}

getESPN_Projections = function(week){
  return(getProjections(paste("C:/MY_FF/Weekly Forecast/ESPN/Projections_Week_",week,"_Date_")));
