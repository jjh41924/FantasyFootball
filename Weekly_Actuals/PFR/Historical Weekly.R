...
#years <- 2014 #2011:2013 #1960:2013
setPFRActuals = function(years) {
  weeks <- 17
  #Scrape data
  qb <- list()
  rb <- list()
  ...
  names(wrDF) <- c("rank","player","age","date","league","team","blank","opponent","result","game","week","day","targets","rec","recYds","ydsPerRec","recTds")
  ...
  weeklyData$home.game = weeklyData$blank == "@"
  extraHeaders = c("pos","age","date","team","home.game","opponent","result","week","day")
  ...
  map = weeklyData[!duplicated(weeklyData[,c("week","name")]),c("name",extraHeaders)]
  weeklyData2 = merge(weeklyData2,map,by=c("name","week"))
  
  #Subset Data
  weeklyData3 <- weeklyData2[,c("name",extraHeaders,"game","year","week","passYds","passTds","passInt","rushYds","rushTds","rec","recYds","recTds")]
