...
setPointsForWeek_FFtoday = function(week){
 ...
 # projections_fftoday[projections_fftoday$name=="BENWATSON", "name"] <- "BENJAMINWATSON"
  ...
  #Save file  
  save(projections_fftoday, file = paste(getMYFFDir(),"/Weekly Forecast/FFtoday/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_fftoday, file=paste(getMYFFDir(),"/Weekly Forecast/FFtoday/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
...
getFFtoday_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/FFtoday/Projections_Week_",week,"_Date_",sep=""))
  ret$source = "FFtoday"
  ret$week = week
  return(ret)
