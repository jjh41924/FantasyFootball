...
setPointsForWeek_ffn = function(week){  
  ...
  #Save file
  #Save file  
  save(projections_ffn, file = paste(getMYFFDir(),"/Weekly Forecast/FFNerd/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_ffn, file=paste(getMYFFDir(),"/Weekly Forecast/FFNerd/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}

getFFNerd_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/FFNerd/Projections_Week_",week,"_Date_",sep=""))
  ret$source = "FFNerd"
  ret$week = week
  return(ret)
