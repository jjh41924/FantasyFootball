...
setPointsForWeek_yahoo = function(week){
  ...
  #Save file  
  save(projections_yahoo, file = paste(getMYFFDir(),"/Weekly_Forecast/Yahoo/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_yahoo, file=paste(getMYFFDir(),"/Weekly_Forecast/Yahoo/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
  
  cat(paste("[Yahoo] DONE. week[",week,"]\n",sep=,""))
}

getYahoo_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly_Forecast/Yahoo/Projections_Week_",week,"_Date_",sep=""))
  ret$source = "Yahoo"
  ret$week = week
  return(ret)
