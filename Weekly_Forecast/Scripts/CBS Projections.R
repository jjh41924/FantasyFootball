...
setPointsForWeek_cbs = function(week){#, isDave = FALSE
  ...
  save(projections_cbs, file = paste(getMYFFDir(),"/Weekly_Forecast/CBS/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_cbs, file=paste(getMYFFDir(),"/Weekly_Forecast/CBS/Projections_Week_",week,"_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
...
#   folder = "CBS_Jamey"
#   if(isDave) { folder = "CBS_Dave" }
  ret = getProjections(paste(getMYFFDir(),"/Weekly_Forecast/CBS/Projections_Week_",week,"_Date_",sep=""))
  ret$source = "CBS"
  ret$week = week
