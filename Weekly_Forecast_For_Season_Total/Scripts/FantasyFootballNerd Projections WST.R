...
setPointsForEntireSeason_ffnerds = function(season=2015) {
  #Suffix
  suffix <- "ffn"
  ...
  #Save file
  save(projections_ffn, file = paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/FFNerd/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_ffn, file=paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/FFNerd/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getFFNerdSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly_Forecast/FFNerd/Projections_Date_",sep=""))
  ret$source = "FFNerd"
  ret$week = week
  return(ret)
