...
setPointsForEntireSeason_fantasysharks = function(season=2015) {
  #Suffix
  suffix <- "fs"
  ...
  #Save file
  save(projections_fs, file = paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/FantasySharks/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_fs, file=paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/FantasySharks/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getFantasySharksSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly_Forecast/FantasySharks/Projections_Date_",sep=""))
  ret$source = "FantasySharks"
  ret$week = week
  return(ret)
