...
#       browser()
  ...
  #Save file
  save(projections_yahoo, file = paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/Yahoo/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_yahoo, file=paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/Yahoo/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}
