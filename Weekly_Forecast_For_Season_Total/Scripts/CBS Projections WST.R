...
setPointsForEntireSeason_cbs = function(season=2015) {
  #Projection Info
  suffix <- "cbs"
  ...
  #Save file
  save(projections_cbs, file = paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/CBS/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_cbs, file=paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/CBS/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}

getCBSSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly_Forecast/CBS/Projections_Date_",sep=""))
  ret$source = "CBS"
  ret$week = week
  return(ret)
