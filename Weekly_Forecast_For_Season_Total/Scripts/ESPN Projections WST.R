...
setPointsForEntireSeason_ESPN = function(season=2015) {
  ...
  #Save file
  save(projections_espn, file = paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/ESPN/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_espn, file=paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/ESPN/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getESPNSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly_Forecast/ESPN/Projections_Date_",sep=""))
  ret$source = "ESPN"
  ret$week = week
  return(ret)
