...
setPointsForEntireSeason_fantasypros = function(season=2015) {
  #Suffix
  suffix <- "fp"
  ...
  #Save file
  save(projections_fp, file = paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/FantasyPros/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_fp, file=paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/FantasyPros/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getYahooSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly_Forecast/Yahoo/Projections_Date_",sep=""))
  ret$source = "FantasyPros"
  ret$week = week
  return(ret)
