...
setPointsForEntireSeason_fox = function(season=2015) {
  ...
  #Save file
  save(projections_fox, file = paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/FOX/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_fox, file=paste(getMYFFDir(),"/Weekly_Forecast_For_Season_Total/FOX/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getFOXSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly_Forecast/FOX/Projections_Date_",sep=""))
  ret$source = "fox"
  ret$week = week
  return(ret)
