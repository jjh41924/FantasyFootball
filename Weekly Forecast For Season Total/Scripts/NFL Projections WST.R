...
setPointsForEntireSeason_nfl = function(season=2015) {
  #Suffix
  suffix <- "nfl"
  ...
  #Save file
  save(projections_nfl, file = paste(getMYFFDir(),"/Weekly Forecast For Season Total/NFL/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_nfl, file=paste(getMYFFDir(),"/Weekly Forecast For Season Total/NFL/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}

getNFLSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/NFL/Projections_Date_",sep=""))
  ret$source = "NFL"
  ret$week = week
  return(ret)
