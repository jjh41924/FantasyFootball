...
setPointsForEntireSeason_cbs = function(season=2015) {
  ...
  #Save file
  save(projections_cbs, file = paste(getMYFFDir(),"/Weekly Forecast For Season Total/CBS/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_cbs, file=paste(getMYFFDir(),"/Weekly Forecast For Season Total/CBS/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}

getCBSSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/CBS/Projections_Date_",sep=""))
  ret$source = "CBS"
  ret$week = week
  return(ret)
