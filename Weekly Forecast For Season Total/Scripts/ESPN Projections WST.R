...
setPointsForEntireSeason_ESPN = function(season=2015) {
  #Projection Info
  suffix <- "espn"
  ...
  #Save file
  save(projections_espn, file = paste(getMYFFDir(),"/Weekly Forecast For Season Total/ESPN/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_espn, file=paste(getMYFFDir(),"/Weekly Forecast For Season Total/ESPN/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getESPNSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/ESPN/Projections_Date_",sep=""))
  ret$source = "ESPN"
  ret$week = week
  return(ret)
