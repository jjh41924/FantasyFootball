...
setPointsForEntireSeason_fantasypros = function(season=2015) {
  ...
  #Save file
  save(projections_fp, file = paste(getMYFFDir(),"/Weekly Forecast For Season Total/FantasyPros/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_fp, file=paste(getMYFFDir(),"/Weekly Forecast For Season Total/FantasyPros/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}
