...
setPointsForEntireSeason_yahoo = function(season=2015) {
  ...
  #Save file
  save(projections_fftoday, file = paste(getMYFFDir(),"/Weekly Forecast For Season Total/FFToday/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_fftoday, file=paste(getMYFFDir(),"/Weekly Forecast For Season Total/FFToday/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getFFTodaySeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/FFToday/Projections_Date_",sep=""))
  ret$source = "FFToday"
  ret$week = week
  return(ret)
