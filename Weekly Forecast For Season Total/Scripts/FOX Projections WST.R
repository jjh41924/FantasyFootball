...
setPointsForEntireSeason_fox = function(season=2015) {
  #Suffix
  suffix <- "fox"
  ...
  #Save file
  save(projections_fox, file = paste(getMYFFDir(),"/Weekly Forecast For Season Total/FOX/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_fox, file=paste(getMYFFDir(),"/Weekly Forecast For Season Total/FOX/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getFOXSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/FOX/Projections_Date_",sep=""))
  ret$source = "fox"
  ret$week = week
  return(ret)
