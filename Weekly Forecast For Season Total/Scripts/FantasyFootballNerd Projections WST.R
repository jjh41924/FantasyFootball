...
setPointsForEntireSeason_ffnerds = function(season=2015) {
  ...
  #Save file
  save(projections_ffn, file = paste(getMYFFDir(),"/Weekly Forecast For Season Total/FFNerd/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_ffn, file=paste(getMYFFDir(),"/Weekly Forecast For Season Total/FFNerd/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getFFNerdSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/FFNerd/Projections_Date_",sep=""))
  ret$source = "FFNerd"
  ret$week = week
  return(ret)
