...
setPointsForEntireSeason_fantasysharks = function(season=2015) {
  ...
  #Save file
  save(projections_fs, file = paste(getMYFFDir(),"/Weekly Forecast For Season Total/FantasySharks/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".RData", sep=""))
  write.csv(projections_fs, file=paste(getMYFFDir(),"/Weekly Forecast For Season Total/FantasySharks/Projections_Date_",strftime(Sys.time(), format = "%Y_%m_%d"),".csv", sep=""), row.names=FALSE)
}


getFantasySharksSeason_Projections = function(week){
  ret = getProjections(paste(getMYFFDir(),"/Weekly Forecast/FantasySharks/Projections_Date_",sep=""))
